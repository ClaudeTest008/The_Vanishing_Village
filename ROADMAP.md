...
## M9 — Third Person, Combat & Visual Overhaul ✅ (M9-3rdPerson-Combat-VisualOverhaul-Complete)
- ✅ Template packs restored from engine (offline, no Fab): Characters (SKM_Manny/Quinn + full unarmed anim set incl. attacks/dash/deaths), Variant_Combat (ABP_Manny_Combat, combo+charged montages, BP_CombatCharacter, StateTree enemy AI, NS_Damage), Variant_Horror (BP_HorrorLight, NS_DustMote), Input, LevelPrototyping
- ✅ Player rebuilt third person: BP_PlayerCharacter reparented onto BP_CombatCharacter_C — inherits mannequin + combat ABP, shoulder camera (380uu boom, lag, y-offset — J-horror over-shoulder), montage combat kit, HP pipeline; all M2–M8 wiring intact. Move/Look rewired to control-rotation Move/Aim, orient-to-movement; walk 500 / sprint 750. GASP itself not on disk (needs Fab download) — combat-variant anims are the mannequin-compatible substitute; motion matching = future swap-in
- ✅ SHf-style melee: stamina 100 (regen 18/s scaled by (1−Pressure/200) — pressure slows recovery in fights; sprint drains 8/s), light LMB (10) combo x3, heavy RMB (25) charged, dodge LAlt (15) = directional 1200uu dash + AM_Dodge + 0.45 s window; perfect dodge (enemy strike lands during window) refunds 20 stamina +25 focus; focus +8/swing, at 100 next heavy = 3x finisher; weapons on PDA_Item (bIsWeapon/WeaponDamage/MaxDurability) equip via inventory Use, per-swing durability, break removes item (DA_WoodcuttersAxe 3/12, DA_SplinteredPlank 1.5/6); WBP_CombatBars stamina/focus HUD
- ✅ Enemies combat-enabled through AC_Ghost: HP 10, hit-stagger 0.5 s, death = NS_Damage burst −15 pressure + despawn; BP_Villager_Base implements BPI_Damageable so player melee traces hurt all residents; attack windup 0.4 s → strike ≤240uu (dodge-aware), Vengeful 14 dmg / 1.4 s (Present yōkai), Distorted 7 dmg / 2.2 s (Past cultists); catch-teleport replaced by real combat — player death (HP 0) flips to Past +25 pressure (curse-won't-let-him-die)
- ✅ Village art pass (425 actors, era-tagged): pine forest ring + clumps, ivy overgrowth on every building, grass/cattails, broken street fences, red torii gate + stone lanterns on shrine approach, marketplace stalls (goods Past / wreckage Present), dinner-table tableaux (dishes Past / decay Present), 3 doll+rocking-chair scenes, 5 yōkai idol statues + debris + tunnel barricade (Present), flags/flowers/log piles (Past), 8 flickering horror lights, 4 dust-mote emitters — all 50/51 era props verified flipping with the Mirror
- ✅ Yōkai visual upgrade: M_YokaiInk (near-black translucent, red fresnel edge, sine WPO shimmer) auto-applied on Vengeful via OnStateChanged; pulsing emissive eye spheres (M_EyeGlow) appear on Vengeful; Past Echo keeps spectral white
- ✅ Full PIE integration: LMB combat via real input, mirror era flips drive villager states + materials + props + pressure, yōkai kill (3 axe hits) & player death loop, weapon equip 1→4 dmg → break → 1, 0 Blueprint runtime errors in final session
- Fixed during test: GetEffectiveBaseDamage SelectFloat evaluated the None branch (82 AccessNone) — replaced object deref with CachedWeaponDamage float set on equip/break
- Deferred: motion matching (GASP via Fab), vaulting/foot IK, IA_ToggleCameraSide mapping, aura Niagara on yōkai, weapon-break SFX, hands-on input feel pass
